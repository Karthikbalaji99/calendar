...
var DatabaseStorage = class {
  async getUser(id) {
    ...
    const { data, error } = await supabase.from("users").select("id, username, password").eq("id", id).maybeSingle();
    if (error) throw error;
    return data ? data : void 0;
  }
  async getUserByUsername(username) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("users").select("id, username, password").eq("username", username).maybeSingle();
    if (error) throw error;
    return data ? data : void 0;
  }
  async createUser(insertUser) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("users").insert(insertUser).select("id, username, password").single();
    if (error) throw error;
    return data;
  }
  ...
  async getAllGratitudeLogs() {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("gratitude_logs").select("*").order("created_at", { ascending: true });
    if (error) throw error;
    return (data || []).map(mapGratitude);
  }
  async createGratitudeLog(log2) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("gratitude_logs").insert(log2).select("*").single();
    if (error) throw error;
  ...
  async getAllHabitCheckins() {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("habit_checkins").select("*").order("date", { ascending: true });
    if (error) throw error;
    return (data || []).map(mapHabitCheckin);
  }
  async createOrUpdateHabitCheckin(checkin) {
    const supabase = getSupabase();
    const { data, error } = await supabase.from("habit_checkins").upsert(
      {
...
var ownerSchema = z.enum(["panda", "cookie", "both"]);
...
var insertMemorySchema = z.object({
  imageUrl: z.string().url().refine((v) => /^https?:\/\//i.test(v), { message: "imageUrl must be http(s) URL" }),
  caption: z.string().optional().default(""),
  date: dateString,
  owner: ownerSchema
});
var memorySchema = insertMemorySchema.extend({
  id: idSchema,
  createdAt: z.string()
});
var insertGratitudeLogSchema = z.object({
  date: dateString,
  from: z.enum(["panda", "cookie"]),
  to: z.enum(["panda", "cookie"]),
  content: z.string().min(1)
});
var gratitudeLogSchema = insertGratitudeLogSchema.extend({
  id: idSchema,
  createdAt: z.string()
});
var insertJournalEntrySchema = z.object({
  date: dateString,
...
var journalEntrySchema = insertJournalEntrySchema.extend({
  id: idSchema,
  createdAt: z.string()
});
...
var taskSchema = insertTaskSchema.extend({
  id: idSchema,
  createdAt: z.string(),
  completed: z.boolean()
});
var insertHabitSchema = z.object({
  name: z.string().min(1),
  owner: ownerSchema,
  color: z.string().min(1)
});
var habitSchema = insertHabitSchema.extend({
  id: idSchema,
  createdAt: z.string()
});
var insertHabitCheckinSchema = z.object({
  habitId: idSchema,
  date: dateString,
  completed: z.boolean().optional().default(false)
});
var habitCheckinSchema = insertHabitCheckinSchema.extend({
  id: idSchema,
  createdAt: z.string()
});
...
async function registerRoutes(app2) {
  ...
  app2.post("/api/upload", upload.single("file"), async (req, res) => {
    try {
      ...
      const supabase = getSupabase();
      const { data, error } = await supabase.storage.from("memories").upload(filename, req.file.buffer, { contentType: req.file.mimetype, upsert: false });
      if (error) {
  ...
  app2.post("/api/gratitude", async (req, res) => {
    try {
      ...
      const log2 = await storage.createGratitudeLog(validatedData);
      res.json(log2);
    } catch (error) {
...
function serveStatic(app2) {
  const moduleDir = typeof __dirname !== "undefined" ? __dirname : path2.dirname(fileURLToPath(import.meta.url));
  const candidates = [
    path2.resolve(process.cwd(), "dist", "public"),
    path2.resolve(moduleDir, "public"),
    path2.resolve(moduleDir, "..", "dist", "public"),
    path2.resolve(moduleDir, "..", "..", "dist", "public")
  ];
  const distPath = candidates.find((p) => fs.existsSync(p));
  if (!distPath) {
    app2.get("*", (_req, res) => {
      res.status(500).set({ "Content-Type": "text/plain" }).end(
